...
- 课表唯一数据源是 `assets/js/schedule.js`（集中结构：周一至周五分日制模板 + 周五晚间特例 + 周六特殊表 + 周日休息），改课表只改这一个文件；`npm run check:schedule` 会跑 81 个边界用例（含 06:50/07:25/22:30 等临界点、周一升旗/周五分段晚补特例、周六/周日特例与 CST 换算）。
...
- 注意：周一至周四 17:30–19:00 是合并的「晚辅导」条目（原始描述只给了这一个时间段，未虚构拆分），周五两节晚补有精确时间故分段；晚餐统一为 19:00–19:40，随后是夜自习一（19:40–21:20）与夜自习二（21:30–22:30）；周六午读（数学）没有给出时间，按无固定时间备注展示。
